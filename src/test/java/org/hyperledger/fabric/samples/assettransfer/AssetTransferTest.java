...
public final class AssetTransferTest {
    ...
    @Nested
    class InvokeDuplicateTransaction {
        @Test
        public void whenAssetDoesNotExist() {
            AssetTransfer contract = new AssetTransfer();
            Context ctx = mock(Context.class);
            ...
            Throwable thrown = catchThrowable(() -> {
                contract.DuplicateAsset(ctx, "asset1_test", "Michel");
            });

            assertThat(thrown).isInstanceOf(ChaincodeException.class).hasNoCause()
                    .hasMessage("Asset not exists");
            assertThat(((ChaincodeException) thrown).getPayload()).isEqualTo("ASSET_NOT_FOUND".getBytes());
        }
        ...
        public void whenAssetExists() {
            ...
            Asset assetNew = contract.DuplicateAsset(ctx, "asset1", "Michel");

            assertThat(assetNew).isEqualTo(new Asset("asset1_duplicate", "blue", 5, "Michel", 300));
        }
