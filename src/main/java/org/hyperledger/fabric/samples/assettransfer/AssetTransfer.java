...
public final class AssetTransfer implements ContractInterface {
    ...
    public Asset DuplicateAsset(final Context ctx, final String assetID, final String owner) {
        final String color;
        final String assetIDnew;
        final int size;
        final int appraisedValue;
        final Asset assetOld;
        final Asset asset;

        ChaincodeStub stub = ctx.getStub();
        System.out.println(AssetExists(ctx, assetID));
        if (AssetExists(ctx, assetID)) {
            assetIDnew = assetID + "_duplicate";
            assetOld = ReadAsset(ctx, assetID);
            color = assetOld.getColor();
            size = assetOld.getSize();
            appraisedValue = assetOld.getAppraisedValue();
            asset = new Asset(assetIDnew, color, size, owner, appraisedValue);
            //Use Genson to convert the Asset into string, sort it alphabetically and serialize it into a json string
            String sortedJson = genson.serialize(asset);
            stub.putStringState(assetID, sortedJson);
            ...
            String errorMessage = String.format("Asset not exists", assetID);
            System.out.println(errorMessage);
            throw new ChaincodeException(errorMessage, AssetTransferErrors.ASSET_NOT_FOUND.toString());
        }
    }
